...
{
	return cpu->pc + getByteAt(cpu->memory, cpu->pc + 1);
}
...
{
	return getByteAt(cpu->memory, cpu->pc + 1);
}

/* zero page x index */
uint16_t resolve_zx(Processor6502* cpu)
{
	return getByteAt(cpu->memory, cpu->pc + 1) + cpu->x;
}

/* zero page y index */
uint16_t resolve_zy(Processor6502* cpu)
{
	return getByteAt(cpu->memory, cpu->pc + 1) + cpu->y;
}

/* absolute */
uint16_t resolve_a(Processor6502* cpu)
{
	return getLEWordAt(cpu->memory, cpu->pc + 1);
}

/* absolute x index */
uint16_t resolve_ax(Processor6502* cpu)
{
	return getLEWordAt(cpu->memory, cpu->pc + 1) + cpu->x;
}

/* absolute y index */
uint16_t resolve_ay(Processor6502* cpu)
{
	return getLEWordAt(cpu->memory, cpu->pc + 1) + cpu->y;
}

/* indirect */
uint16_t resolve_i(Processor6502* cpu)
{
	uint16_t address1 = getLEWordAt(cpu->memory, cpu->pc + 1);
